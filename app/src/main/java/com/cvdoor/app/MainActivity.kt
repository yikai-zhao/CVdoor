...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            CVDoorTheme {
                Surface(modifier = androidx.compose.ui.Modifier.statusBarsPadding()) {
                    CVATSApp()
                }
            }
        }
...
fun CVATSApp() {
    ...
    // V2 flow starts from landing; legacy pages remain available
    NavHost(navController = nav, startDestination = "landing") {
        ...
        composable("landing") {
            LandingScreen(
                onStart = { nav.navigate("resume_select") }
            )
        }
        ...
        // ── Legacy Dashboard (signed-in only) ────────────────────────────────

        composable("signin") {
            com.cvdoor.app.ui.screens.SignInScreen(
                onSignedIn = {
                    nav.navigate("home") { popUpTo("signin") { inclusive = true } }
                }
            )
        }
        ...
        composable(
            ...
            if (rec != null) {
                ...
                androidx.compose.material3.Text("Record not found (id=$id)")
            }
        }
